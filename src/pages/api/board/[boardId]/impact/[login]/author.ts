...
import { getAuthorMetrics } from "@/lib/services/impact-metrics";
...
export const GET: APIRoute = async (context) => {
  ...
  if (board.role !== "supervisor") {
    const { data: own, error: ownError } = await supabase
      ...
      .maybeSingle();
    if (ownError) {
      logger.error("[impact/author] own-contributor lookup failed", ownError);
  ...
  if (contribError) {
    logger.error("[impact/author] contributor lookup failed", contribError);
    return json({ error: "Database error" }, 500);
  }
  if (!contributor) return json({ error: "Contributor not found" }, 404);

  try {
    const result = await getAuthorMetrics(
      supabase,
      boardId,
    ...
    logger.error("[impact/author] service error", err);
