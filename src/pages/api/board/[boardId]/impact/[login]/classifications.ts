...
import { getClassificationAggregates } from "@/lib/services/impact-metrics";
...
export const GET: APIRoute = async (context) => {
  ...
  if (contribError) {
    logger.error("[impact/classifications] contributor lookup failed", contribError);
  ...
  if (board.role !== "supervisor") {
    const ownProfile = await getUserProfile(supabase, user.id).catch((err: unknown) => {
      logger.error("[impact/classifications] own-profile lookup failed", err);
      return null;
    });
  ...
  try {
    const result = await getClassificationAggregates(
      supabase,
      boardId,
    ...
    logger.error("[impact/classifications] service error", err);
