-- profile-classified-comments: "started" on the Threads page must exclude comments the
-- contributor left on their own PR (self-review), to match Impact's threadsStarted KPI, which
-- already excludes self-authored-PR comments. "all" is untouched on purpose — it already covers
-- self-review via its unconditional commenter-match clause, so the self-review intent filter
-- keeps working.
