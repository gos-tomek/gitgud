-- profile-classified-comments: paginated/filterable classified thread list for the Threads page.
-- Same URL-length-safety rationale as get_board_reviews_for_reviewer /
-- get_board_classifications_for_commenter — joins on repo_id server-side rather than taking a
-- caller-supplied PR-id array. Filtering (intent/domain/PR/role) and pagination happen in SQL so
-- the API layer never has to fetch a contributor's full thread history just to slice a page.
--
-- p_role selects the perspective: 'started' (threads the contributor opened), 'received'
-- (threads opened by others on the contributor's own PRs), or 'all' (union of both).
--
-- total_count is a window-function column (same value repeated on every row) so the caller gets
-- the page's row set and the unpaginated total in a single round trip.
--
