...
- **Secrets**: `SUPABASE_URL`, `SUPABASE_KEY`, `SUPABASE_SERVICE_KEY`, and `GITHUB_TOKEN_ENCRYPTION_KEY` live in **Workers Secrets** (`wrangler secret put <NAME>`, encrypted at rest) for runtime. `SUPABASE_URL`/`SUPABASE_KEY` are also **GitHub Actions repository secrets** for the CI build step. Never commit them; `.dev.vars` (gitignored) holds them for local dev.
  - `SUPABASE_URL`/`SUPABASE_KEY` (anon) — read via `astro:env/server`, used by the cookie-based SSR client for every request-scoped operation.
  - `SUPABASE_SERVICE_KEY` (service-role, bypasses RLS) — read directly from `env` (not `astro:env/server`) by the Worker's `scheduled()` cron dispatcher and the `ClassificationBatchWorkflow` (`src/worker.ts`), which run outside a user session and can't use the cookie-based client.
  - `GITHUB_TOKEN_ENCRYPTION_KEY` — the pgcrypto symmetric passphrase `create_board_atomic` uses to encrypt the GitHub PAT on board creation, and that the Workflow uses to decrypt it before calling the GitHub API. It has no SQL `DEFAULT`, so if the Worker secret is missing, PostgREST can't resolve the RPC's argument signature and board creation fails with `PGRST202` (incident: 2026-06-22, see deployment-plan.md Phase 6).
  - Rotation = re-run `wrangler secret put <NAME>` for any of the four.
...
4. **Set runtime secrets**: `npx wrangler secret put SUPABASE_URL`, `npx wrangler secret put SUPABASE_KEY`, `npx wrangler secret put SUPABASE_SERVICE_KEY`, then `npx wrangler secret put GITHUB_TOKEN_ENCRYPTION_KEY`.
