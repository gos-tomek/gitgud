...
- [ ] `npx wrangler tail` in a second terminal to stream live logs during the checks below.
- [ ] Open the workers.dev URL: home page (`/`) renders (SSR).
- [ ] **Supabase Auth dashboard → URL Configuration:** set **Site URL** and add the workers.dev URL to **Redirect URLs**. Without this, signup confirmation emails and signin redirects break (`/auth/signup`, `/auth/confirm-email`). External-integration edge case — easy to miss.
- [ ] Sign up → receive confirmation email → confirm → land authenticated.
- [ ] Sign in, then hit `/dashboard` (the only `PROTECTED_ROUTES` entry, `src/middleware.ts`) — authed access works; signed-out access redirects.
