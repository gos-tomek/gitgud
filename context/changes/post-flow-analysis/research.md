...
| 11  | Threads UI                     | `GET /api/board/[boardId]/threads/[login]                                                    | [threadId]`    | board member                                                                                         | `src/pages/api/board/[boardId]/threads/*.ts` → `impact-metrics.ts` |
...
- **Read side is thick, write side is thin.** `impact-metrics.ts` (1150 lines, 30+ column reads) is far larger than `github-sync.ts` (620 lines) — the pipeline stores what GitHub gives, all business logic (velocity, throughput, classifications aggregation) is at read time.
...
| 15  | Hardcoded sync URLs at `SyncIndicator.tsx:46` and `:62`                                                     | ✅ **Confirmed**                 | `:46` `\`/api/github/sync/status?boardId=${boardId}&instanceId=${instanceId}\``, `:62` `"/api/github/sync"`. No other file in `src/` references these two URLs                                                                                                                                                                                                                                                                                                                                                                                                                                             |
