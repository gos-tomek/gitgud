...
**b) Server-side guarantee** — Workstream 1's ruleset makes a direct push to `main` _fail_
...
> editing shared skill behavior adds nothing. If "never _attempt_ the push" matters, the optional
...
**Runtime Worker secrets stay out of CI.** `infrastructure.md` distinguishes _build_ secrets (GitHub
repo secrets, used above) from _runtime_ secrets set on the Worker via `wrangler secret put`
...
the migration must not break the currently-running _old_ code (there's a window where new schema +
...
(If the user later wants roadmap fully automated, do it from a _separate_ scheduled/labelled
